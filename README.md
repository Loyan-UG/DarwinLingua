...
- `Phase 5 server content distribution`: `In Progress`
  - the backend foundation now includes a Web API host, PostgreSQL-backed manifest persistence, package download endpoints, schema compatibility checks, local PostgreSQL/Web API development templates, a server-side import workflow that stages draft package batches plus an explicit publish endpoint that promotes drafts to published delivery, lightweight admin endpoints to inspect draft/published batches, and mobile Settings flows for full, catalog, and `A1`-`C2` remote updates with local-state-preserving apply logic and richer scope/checksum/schema diagnostics
