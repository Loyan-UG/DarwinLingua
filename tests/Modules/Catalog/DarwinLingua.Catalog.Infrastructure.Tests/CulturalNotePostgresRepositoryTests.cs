...
public sealed class CulturalNotePostgresRepositoryTests
{
    private const string DefaultDockerContainerName = "darwinlingua-postgres";
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    [Fact]
    public async Task CulturalNoteRepository_ShouldFilterOrderAndProjectLocalizedHelpers()
    {
        ...
        try
        {
            ...
            await using (DarwinLinguaDbContext dbContext = await dbContextFactory.CreateDbContextAsync(CancellationToken.None))
            {
                ...
                dbContext.CulturalNotes.AddRange(
                    CreateNote(nowUtc),
                    CreateNote(
                        nowUtc,
            ...
            ICulturalNoteRepository repository = serviceProvider.GetRequiredService<ICulturalNoteRepository>();
            IReadOnlyList<CulturalNoteListItemModel> items = await repository.GetPublishedCulturalNotesAsync(
                new CulturalNoteListFilterModel("B1", "democracy-and-state", "Mitbestimmung", "DEMOKRATIE"),
                "fa",
                CancellationToken.None);

            CulturalNoteListItemModel item = Assert.Single(items);
            ...
            CulturalNoteDetailModel? detail = await repository.GetPublishedCulturalNoteBySlugAsync(
            ...
            IReadOnlyList<UnifiedLearningSearchResultModel> searchResults = await searchRepository.SearchAsync(
                new UnifiedLearningSearchFilterModel("Demokratie", "B1", "cultural-note", "democracy-and-state", null),
                CancellationToken.None);

            UnifiedLearningSearchResultModel result = Assert.Single(searchResults);
            Assert.Equal("cultural-note", result.ResultType);
            Assert.Equal("/life-in-germany/b1-demokratie-im-alltag-verstehen", result.Url);
        }
        finally
        {
    ...
    private static CulturalNote CreateNote(
        DateTime nowUtc,
