...
- The Web readiness checkpoint for the current Learning Portal baseline is closed locally. On 2026-06-14, targeted Web/WebApi builds and Phase 7 tests passed; in-app Chromium manual smoke covered learner routes, account/admin authorization redirects, Unified Search, `/recent`, and a 390px mobile viewport without horizontal overflow. Shared PostgreSQL counts matched the baseline: `CourseLessons=560`, `WritingTemplates=120`, `ExamPrepUnits=246`, and `CulturalNotes=30`. On 2026-06-23, the current Brevo-ready tester bundle `artifacts/validation/web-tester-runs/20260623-171610-web-tester-pass-brevo-ready` passed 25/25 public-routed preflight checks, including browse, word detail, favorites, settings, anonymous admin login redirects, public API health, Course Persian helper projection, and representative search queries. The empty feedback CSV triage dry-run passed with 0 validation errors at `artifacts/validation/web-tester-feedback/web-tester-feedback-triage-20260623-171624.md`. The latest restore-ready external checkpoint is `X:\Projects\DarwinLingua.Backup\20260623-164407-web-transactional-email-preview-ready-pre-user-testing`.
- Transactional email has a Brevo API provider path, sandbox behavior, webhook event handling, delivery logs, diagnostics, suppression handling, branded HTML/plain-text templates, safe local template previews, and a configured Brevo-ready controlled public Web stack. Manual mailbox rendering review and ongoing provider event monitoring remain operational checks before broad public launch.
...
3. Use Brevo-ready self-registration for the first controlled Web tester pass. Use pre-created accounts only for exceptions, and grant Premium in batch only after the tester accounts already exist.
4. Invite the first controlled Web testers using `artifacts/validation/web-tester-runs/20260623-171610-web-tester-pass-brevo-ready/TesterQuickStart.md`, collect observations in that bundle's `WebTesterFeedback.csv`, and run the triage helper before new bulk content work.
