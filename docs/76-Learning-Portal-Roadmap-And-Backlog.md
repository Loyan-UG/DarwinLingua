...
1. Add the current host IP from `artifacts/validation/web-external-action-packet/web-external-action-packet-20260624-000221.md` in Brevo `Security -> Authorised IPs`, then rerun `tools/Web/Invoke-BrevoProductionReadinessCheck.ps1 -VerifyBrevoApi -RequireRealDelivery -SenderVerified -DnsAuthenticated -WebhookConfigured -DpaAccepted`.
2. Close the human evidence gates in `docs/91-Web-Manual-External-Review-Checklist.md`: mailbox rendering, desktop PWA install, Android PWA install or explicit not-in-scope decision, tester-pass start status, and controlled tester pass evidence.
3. Rerun `tools/Web/New-WebControlledTesterReadinessAudit.ps1 -FailOnAutomatedFailure -FailOnOpenHumanGates`; invite testers only after it passes.
4. Use Brevo-ready self-registration for the first controlled Web tester pass. Use pre-created accounts only for exceptions, and grant Premium in batch only after the tester accounts already exist.
5. Invite the first controlled Web testers using `artifacts/validation/web-tester-runs/20260623-172706-web-tester-pass-brevo-ready/TesterQuickStart.md`, keep `ManualExternalReviewChecklist.md` operator-only, collect observations in that bundle's `WebTesterFeedback.csv`, and run the triage helper before new bulk content work.
6. During tester feedback, only repair broken links, missing translations, route mismatches, smoke blockers, and clear UX/content issues.
7. Treat Course `activityBlocks` backfill as complete for the first Web baseline: A1 `60/60`, A2 `80/80`, B1 `100/100`, B2 `80/80`, C1 `120/120`, and C2 `120/120`.
8. Generate any further Everyday Expressions, RoleplayScenario, Exam Prep, Writing Templates, or Life in Germany content only as small reviewed batches after an explicit plan, strict quality gate, import, Web/API smoke, Unified Search smoke, and admin report checks.
