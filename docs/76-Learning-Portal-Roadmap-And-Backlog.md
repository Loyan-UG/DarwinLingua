...
- Standalone RoleplayScenario infrastructure now has a first Web-first implementation slice: parser support, import validation, shared-content persistence, Web API list/detail routes, Web list/detail pages, Unified Search result support, and admin count visibility. Standalone roleplay pilot generation remains blocked until dedicated import/query/API/Web/admin tests and local shared-database smoke pass.
...
4. Close the remaining RoleplayScenario validation gaps before standalone Roleplay content generation.
...
- [ ] close standalone RoleplayScenario validation before generating standalone roleplay packages
  - Progress: initial Web-first infrastructure now includes `roleplayScenarios` parser support, application validation, persistence, query services, `/api/catalog/roleplays`, `/roleplays`, Unified Search result support, and admin count-by-type/CEFR visibility. Full solution build and the first parser test pass.
  - Remaining gate: add import persistence tests, repository filter/detail tests, WebApi route tests, Web structural tests, admin quality tests, local import/smoke, and a validation artifact before creating `roleplays-a1-b2-pilot-v1.json`.
