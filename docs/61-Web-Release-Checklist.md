...
- Known accepted issues:
  ...
  - Brevo production sending is now configured and smoke-tested for the controlled local public stack with verified provider/API readiness, direct real-delivery smoke, app-level registration/password-reset delivery logs, and a controlled public webhook/suppression smoke. Manual mailbox render review, operator UI diagnostics review, later suppressed-send drill, and ongoing real provider webhook/event monitoring still remain operational checks before broad public launch.
