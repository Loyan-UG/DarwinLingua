...
- [x] transactional email provider processing/DPA requirements are reviewed
  - Evidence: Brevo DPA was accepted by the operator. 2026-06-22 readiness evidence confirms `operator.dpaAccepted`, sender verification, DNS authentication, webhook configuration, API key, webhook secret, and real-delivery settings. The stronger live API report `artifacts/validation/brevo-readiness/brevo-production-readiness-20260622-222809.md` still has one operational blocker: Brevo rejects this host because the current IP is not authorized. Real inbox/webhook smoke must wait until that IP is added in Brevo Authorized IPs.
...
- [x] production email provider configured
  - Evidence: 2026-06-22 Brevo readiness report `brevo-production-readiness-20260622-222809.md` passed `config.mode=BrevoApi`, `secret.apiKey`, `secret.webhookSecret`, `config.sandbox=false`, `config.querySecretFallback=false`, and `operator.webhookConfigured`. The provider is configured, but real delivery smoke remains blocked until Brevo Authorized IPs includes the current host IP.
- [ ] Brevo live API reachability from the current host is verified
  - Evidence needed: rerun `Invoke-BrevoProductionReadinessCheck.ps1` with `-VerifyBrevoApi` and get 0 blockers. Current evidence `brevo-production-readiness-20260622-222809.md` has blocker `brevo.accountApi` because Brevo rejected IP `109.85.65.57` as unauthorized.
