...
- [x] production email provider configured
  - Evidence: 2026-06-23 Brevo readiness report `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-222427.md` passed `config.mode=BrevoApi`, `secret.apiKey`, `secret.webhookSecret`, `config.sandbox=false`, `config.querySecretFallback=false`, and `operator.webhookConfigured`. Provider-side webhook configuration was historically verified by `artifacts/validation/brevo-webhook-configuration-check/brevo-webhook-configuration-check-20260623-191243.md`; rerun that check after the current host IP is added in Brevo Authorized IPs.
- [ ] Brevo live API reachability from the current host is verified
  - Current status: `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-222427.md` was generated with `-SendingDomain darwinlingua.com -RequireRealDelivery -VerifyBrevoApi`; it reports `blockerCount=1`, `warningCount=0`, and the single blocker is Brevo Authorized IP. Historical report `brevo-production-readiness-20260623-185124.md` passed earlier, but it no longer proves reachability from the current host/IP.
