...
- Phase 7 parser tests passed for Grammar, Expressions, Exercises, Courses, Exam Prep, Writing Templates, and Country Guidance.
...
- Country Guidance Germany A1/A2/B1 foundation content is generated and imported in local shared PostgreSQL with `CountryGuidanceNotes=30` (`A1=10`, `A2=10`, `B1=10`) under `targetLearningLanguageCode=de` and `countryContextCode=DE`. The canonical public Web route is `/learn/de/country-guidance/de`; the canonical API route is `/api/catalog/country-guidance/{countryContextCode}`. The internal model/table rename is complete: `CountryGuidanceNote` maps to `CountryGuidanceNotes`, with old naming retained only in historical migrations and explicit rename SQL.
- Web readiness manual smoke on 2026-06-14 passed in the in-app Chromium browser for `/courses`, `/courses/a1-einstieg-in-den-alltag`, `/exercises`, `/exam-prep`, `/exam-prep/profile/goethe-c1`, a C1 exam-prep detail, `/writing-templates`, an A1 writing-template detail, `/life-in-germany`, an A1 Life in Germany detail, `/search?q=Demokratie&resultType=country-guidance`, and `/recent`. No horizontal overflow was detected on desktop or a 390px mobile viewport for the text-heavy detail/search pages. Phase 8 route hardening supersedes those older route examples with `/learn/de/country-guidance/de` and `resultType=country-guidance`.
...
- PWA follow-up on 2026-06-23 regenerated `artifacts/validation/pwa-installability/pwa-installability-darwinlingua-20260623-205307.json` against `https://darwinlingua.com`: `17` automated checks passed, `0` failed, and `2` manual checks remain for real Desktop Chrome/Edge and Android Chrome prompt acceptance. Headless Chromium observed the desktop install-prompt event, but real installed-window behavior still requires target-browser validation.
- Web readiness checkpoint commits are selected through the current controlled tester readiness head: `d31d41b8 Require manual gate evidence for tester readiness`. Earlier Web/Brevo hardening is included in the pushed history, including account email workflow structure, Life in Germany legal research/content gates, Brevo handoff, identity token failures, manual external review, provider-side webhook configuration verification, and the consolidated tester readiness audit.
...
- Brevo production readiness now has repeatable operator gates: `tools/Web/Invoke-BrevoProductionReadinessCheck.ps1` writes JSON/Markdown evidence under `artifacts/validation/brevo-readiness/`; `tools/Web/Invoke-BrevoRealDeliverySmoke.ps1` verifies direct provider delivery; `tools/Web/Invoke-WebAccountEmailFlowSmoke.ps1` verifies registration and password-reset email flow through the public Web UI and `WebEmailDeliveryLogs`; `tools/Web/Invoke-BrevoWebhookSuppressionSmoke.ps1` verifies the public webhook endpoint, Bearer-token authentication, failed delivery-log update, and internal suppression creation without sending to a real recipient; `tools/Web/Invoke-BrevoSuppressedSendSmoke.ps1` verifies that a later account email to a suppressed recipient is logged as `Suppressed` without calling Brevo; `tools/Web/Invoke-BrevoTransactionalLogCheck.ps1` verifies provider-side transactional logs without printing secrets or full provider message ids; and `tools/Web/Invoke-WebEmailDiagnosticsAdminSmoke.ps1` verifies the authenticated Admin Email Diagnostics UI shows provider message ids, provider events, suppression data, readiness state, and Admin-only controls. The production handoff also has a Persian operator guide in `docs/89-Brevo-Operator-Handoff.fa.md`, transactional email templates now render action links as email-safe CTA buttons, and `docs/91-Web-Manual-External-Review-Checklist.md` records the remaining real-mailbox/PWA/tester-pass human evidence gates.
...
- Latest controlled tester readiness audit: `artifacts/validation/web-controlled-tester-readiness/web-controlled-tester-readiness-20260623-234126.md` reports `Automated ready=False`, `Automated failures=1`, `Controlled tester ready to invite=False`, and five human-start open gates: mailbox rendering, desktop PWA install, Android PWA install, tester-pass start status, and controlled tester pass evidence. The automated failure is the current Brevo Authorized IP blocker; `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-234027.md` has `Blockers=1`, `Warnings=0`, with `brevo.accountApi` rejected until the current host IP is added in Brevo `Security -> Authorised IPs`.
...
- [x] release commit selected
  - Evidence: current controlled Web testing readiness head is `9983780c Record controlled tester readiness backup`, with the immediately preceding implementation commit `56d2a6af Add controlled tester readiness audit`. The backup manifest at `X:\Projects\DarwinLingua.Backup\20260623-193207-controlled-tester-readiness-audit-ready-pre-user-testing\manifest.md` records Git commit `56d2a6affbdf7e6819b5b9822586c29c27c2b2d9`; the follow-up documentation commit is in GitHub and does not require a second DB backup.
...
- [x] manifest is delivered
  - Evidence: 2026-06-23 public-domain PWA report `artifacts/validation/pwa-installability/pwa-installability-darwinlingua-20260623.json` returned HTTP 200 and `application/manifest+json` for `https://darwinlingua.com/manifest.webmanifest`; desktop Chromium page inspection loaded the manifest link and parsed `name=Darwin Lingua`, `display=standalone`, `id=/`, and `scope=/`.
- [x] service worker registers
  - Evidence: 2026-06-23 public-domain PWA report registered the service worker at scope `https://darwinlingua.com/`, created cache `darwin-lingua-shell-v3`, and cached `/offline.html`, manifest, and icon assets.
- [x] offline shell fallback is available
  - Evidence: 2026-06-23 public-domain PWA report verified `/offline.html` returned HTTP 200, shell cache existed, and offline shell assets were cached under `https://darwinlingua.com/`.
- [ ] install flow validated on target browsers
- Desktop installability baseline evidence is generated by `tools/Web/New-WebPwaInstallabilityReport.ps1`; the 2026-06-23 public-domain report at `artifacts/validation/pwa-installability/pwa-installability-darwinlingua-20260623-205307.json` passed 17 automated checks with 0 failures and left 2 manual checks for real install prompt acceptance on desktop Chrome/Edge and Android Chrome. The desktop prompt event was observed in headless Chromium, but actual browser acceptance and installed-window behavior remain manual.
...
- [x] email confirmation is sent after registration
  - Evidence: 2026-06-18 `WebRegistrationLegalAcknowledgementTests` verifies registration calls `GenerateEmailConfirmationTokenAsync` and `SendEmailConfirmationAsync`; public smoke returned HTTP 200 for `/Identity/Account/Register`. 2026-06-23 app-level public link smoke `artifacts/validation/web-account-email-link-smoke/web-account-email-link-smoke-20260623-173710.md` registered a test learner through `https://darwinlingua.com`, resolved the Brevo-tracked confirmation email, and confirmed the account through the real action link.
...
- [x] transactional email provider processing/DPA requirements are reviewed
  - Evidence: Brevo DPA was accepted by the operator. 2026-06-23 readiness evidence `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-234027.md` confirms `operator.dpaAccepted`, sender verification, DNS authentication, webhook configuration, API key, webhook secret, sandbox disabled, and query-string fallback disabled. Live Brevo API reachability is not currently closed because `brevo.accountApi` is blocked by Brevo Authorized IP until the current host IP is added.
...
- [ ] production configuration applied
  - Current controlled-test status: the Darwin-domain development/public-test configuration is applied and smoke-tested for `https://darwinlingua.com` and `https://api.darwinlingua.com/health`. This does not close broad production launch configuration, because final hosting, legal sign-off, billing state, and manual tester gates still remain separate.
- [x] database connectivity verified
  - Evidence: 2026-06-18 local Web/WebApi smoke returned `200` for `https://localhost:7501/`, `https://localhost:7501/courses`, and `http://localhost:5099/health`; public smoke returned `200` for `https://lingua.vafadar.pro/`, `/courses`, and `https://linguaapi.vafadar.pro/health`. 2026-06-23 Darwin-domain public stack helper `tools/Web/Start-WebPublicDevStack.ps1 -StopExisting` generated `artifacts/validation/web-public-stack/web-public-stack-20260623-233654.md` with smoke passed for `https://darwinlingua.com`, `/legal`, `/privacy`, `/cookies`, and `https://api.darwinlingua.com/health`. PostgreSQL verification against `darwinlingua_shared` returned `CourseLessons=560`, `WritingTemplates=120`, `ExamPrepUnits=246`, `CountryGuidanceNotes=30`, and `AspNetUsers=7`.
...
- [x] phase-completion backup created under `X:\Projects\DarwinLingua.Backup` with PostgreSQL dumps, repo restore overlay, Git restore bundle, separate local config/secret bundle, manifest, restore notes, and checksum verification
  - Evidence: latest restore-ready Web/domain/Brevo/tester-readiness checkpoint is `X:\Projects\DarwinLingua.Backup\20260623-193207-controlled-tester-readiness-audit-ready-pre-user-testing`. It contains PostgreSQL dump, restore list, globals, repo overlay, `repo-overlay/darwinlingua-current-head.bundle`, separate `secrets/` bundle including ASP.NET user-secrets and local appsettings, Docker metadata, `verification/`, `manifest.md`, and `checksums.sha256`. The manifest records Git commit `56d2a6affbdf7e6819b5b9822586c29c27c2b2d9`, database `darwinlingua_shared`, `Database dump skipped: False`, and `Repo overlay skipped: False`; `docs/60-Web-Operations-Notes.md` records the follow-up documentation commit `9983780c`.
- [x] production email provider configured
  - Evidence: 2026-06-23 Brevo readiness report `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-234027.md` passed `config.mode=BrevoApi`, `secret.apiKey`, `secret.webhookSecret`, `config.sandbox=false`, `config.querySecretFallback=false`, and `operator.webhookConfigured`. Provider-side webhook configuration was historically verified by `artifacts/validation/brevo-webhook-configuration-check/brevo-webhook-configuration-check-20260623-191243.md`; rerun the production readiness check after the current host IP is added in Brevo Authorized IPs, and rerun the webhook configuration check only if webhook metadata changed.
- [ ] Brevo live API reachability from the current host is verified
  - Current status: `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-234027.md` was generated with `-RequireRealDelivery -VerifyBrevoApi -SenderVerified -DnsAuthenticated -WebhookConfigured -DpaAccepted`; it reports `blockerCount=1`, `warningCount=0`, and the single blocker is Brevo Authorized IP. Historical report `brevo-production-readiness-20260623-185124.md` passed earlier, but it no longer proves reachability from the current host/IP.
- [x] Brevo real inbox/provider smoke is sent
  - Evidence: `artifacts/validation/brevo-real-delivery-smoke/brevo-real-delivery-smoke-20260623-173204.md` reports `Status: sent` for `Account.EmailConfirmation` and `Account.PasswordReset` to `info@darwinlingua.com`, with Brevo provider message ids recorded. 2026-06-23 app-level Web link smoke `artifacts/validation/web-account-email-link-smoke/web-account-email-link-smoke-20260623-173710.md` exercised registration, resend confirmation, password reset, and email change through public Web and real Brevo-tracked links. `artifacts/validation/brevo-webhook-suppression-smoke/brevo-webhook-suppression-smoke-20260623-173642.md` and `artifacts/validation/brevo-transactional-log-check/brevo-transactional-log-check-20260623-173654.md` passed. Manual inbox visual rendering review should still confirm how the messages look in real mail clients before broad tester self-registration.
...
- [x] email provider processing/DPA requirements reviewed where applicable
  - Evidence: 2026-06-23 Brevo readiness report passed `operator.dpaAccepted`; `docs/89-Brevo-Operator-Handoff.fa.md` records the Brevo DPA and webhook event setup.
...
- [x] Country Guidance Germany readiness reviewed: category/context filters, neutral/safe content handling, WebApi/Web pages, linked-content behavior
  - Evidence: Germany guidance A1/A2/B1 foundation packages are imported into `darwinlingua_shared` with `CountryGuidanceNotes=30` under `targetLearningLanguageCode=de` and `countryContextCode=DE`; public Web route is `/learn/de/country-guidance/de`, API is `/api/catalog/country-guidance/{countryContextCode}`, and targeted PostgreSQL repository coverage verifies stable filtering, helper projection, linked slugs, and Unified Search URLs.
- [x] Unified Search readiness reviewed: deterministic ranking, filters, result-type labels, empty-state behavior, preservation of existing word search
  - Evidence: seeded repository/API/Web structural coverage verifies deterministic ranking, filters, labels, safe missing references, and bounded performance; Phase 8 Country Guidance search now uses `resultType=country-guidance`, with result URLs under `/learn/de/country-guidance/de/{slug}`.
...
- [x] Controlled Web tester onboarding runbook exists
  - Evidence: `docs/87-Web-Tester-Onboarding-Runbook.md` defines tester profiles, repeatable pre-test smoke via `tools/Web/Invoke-WebTesterPreflight.ps1`, task scripts for Course flow, helper languages, Writing Templates, Exam Prep, Life in Germany, Search/Recent, feedback fields, triage rules, and completion criteria. The current Brevo-ready public tester bundle `artifacts/validation/web-tester-runs/20260623-172706-web-tester-pass-brevo-ready` passed 25/25 checks on `https://darwinlingua.com` and `https://api.darwinlingua.com`, includes `ManualExternalReviewChecklist.md`, and covers learner routes, anonymous admin login redirects, public API health, Course Persian helper projection, and representative search queries.
...
- [x] Controlled Web tester validation bundle helper exists
  - Evidence: `tools/Web/New-WebTesterValidationBundle.ps1` creates a timestamped tester-pass folder with the operator runbook, tester quick start, manual external review checklist, feedback CSV, manifest, README, and optional pass-specific preflight report. The current public-routed tester bundle was generated at `artifacts/validation/web-tester-runs/20260623-172706-web-tester-pass-brevo-ready` with preflight status `passed` and 25/25 checks green for public learner routes, anonymous admin login redirects, public API health, Course Persian helper projection, and representative search queries.
- [x] Controlled Web tester readiness audit exists
  - Evidence: `tools/Web/New-WebControlledTesterReadinessAudit.ps1` consolidates the latest public stack, operations bootstrap, Brevo readiness, webhook configuration, delivery/link/suppression/log smokes, Admin Email Diagnostics smokes, tester bundle preflight, and manual external-review report. It intentionally separates `Automated ready` from `Controlled tester ready to invite`, does not require `www.darwinlingua.com`, carries manual-report evidence gaps into the start gate, and does not approve broad public launch while mailbox/PWA/tester human gates remain open.
...
- Known accepted issues:
  - This is not a broad public production launch sign-off. The primary Web test domain is `https://darwinlingua.com`; `https://www.darwinlingua.com` is configured only as a canonical redirect to the apex domain and is not a separate required route.
  - Brevo production sending is configured and has prior controlled real-delivery/webhook evidence, but the current production readiness gate is not closed while Brevo rejects the current host IP. Add the current host IP in Brevo `Security -> Authorised IPs`, rerun `Invoke-BrevoProductionReadinessCheck.ps1`, then refresh the controlled tester readiness audit. Manual mailbox render review and ongoing real provider webhook/event monitoring still remain operational checks before broad public launch.
...
- Final release recommendation: proceed with the controlled Web tester pass only. Do not treat this checkpoint as approval for broad public launch until tester feedback, final legal/operator review, manual mailbox rendering review, PWA install validation, and any resulting blocker/major fixes are closed with evidence.
