...
- Web readiness checkpoint commits are selected through the current controlled tester readiness head: `9983780c Record controlled tester readiness backup`. Earlier Web/Brevo hardening is included in the pushed history, including account email workflow structure, Life in Germany legal research/content gates, Brevo handoff, identity token failures, manual external review, provider-side webhook configuration verification, and the consolidated tester readiness audit.
...
- Latest controlled tester readiness audit: `artifacts/validation/web-controlled-tester-readiness/web-controlled-tester-readiness-20260623-232858.md` reports `Automated ready=False`, `Automated failures=1`, `Controlled tester ready to invite=False`, and four open human gates: mailbox rendering, desktop PWA install, Android PWA install, and tester-pass start status. The automated failure is the current Brevo Authorized IP blocker; `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-232247.md` has `Blockers=1`, `Warnings=0`, with `brevo.accountApi` rejected until the current host IP is added in Brevo `Security -> Authorised IPs`.
