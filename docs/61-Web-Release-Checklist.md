...
- [x] transactional email provider processing/DPA requirements are reviewed
  - Evidence: Brevo DPA was accepted by the operator. 2026-06-22 readiness evidence confirms `operator.dpaAccepted`, sender verification, DNS authentication, webhook configuration, API key, webhook secret, and real-delivery settings. The stronger live API report `artifacts/validation/brevo-readiness/brevo-production-readiness-20260622-223909.md` still has one operational blocker: Brevo rejects this host because IP `109.85.65.57` is not authorized. Real inbox/webhook smoke must wait until that IP is added in Brevo Authorized IPs.
