...
- [x] phase-completion backup created under `X:\Projects\DarwinLingua.Backup` with PostgreSQL dumps, repo restore overlay, Git restore bundle, separate local config/secret bundle, manifest, restore notes, and checksum verification
  - Evidence: latest Web/domain/Brevo/manual-review checkpoint is `X:\Projects\DarwinLingua.Backup\20260623-180250-web-brevo-live-ready-manual-review-pre-user-testing`. It contains PostgreSQL dump, restore list, globals, repo overlay, separate `secrets/` bundle including ASP.NET user-secrets and local appsettings, Docker metadata, `manifest.md`, and `checksums.sha256`. The manifest records Git commit `da2818df671d1586aaa8d843e38db1f23aa7d924`, database `darwinlingua_shared`, `Database dump skipped: False`, and `Repo overlay skipped: False`.
- [x] production email provider configured
  - Evidence: 2026-06-23 Brevo readiness report `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-173205.md` passed `config.mode=BrevoApi`, `secret.apiKey`, `secret.webhookSecret`, `config.sandbox=false`, `config.querySecretFallback=false`, `operator.webhookConfigured`, and `brevo.accountApi` with `0` blockers and `0` warnings.
- [x] Brevo live API reachability from the current host is verified
  - Evidence: `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-173205.md` was generated with `-VerifyBrevoApi -RequireRealDelivery`, `blockerCount=0`, and `warningCount=0`.
