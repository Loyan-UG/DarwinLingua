...
Course imports are safe for both cumulative full-path packages and reviewed module-slice packages. When a package contains an existing course path slug, the importer upserts the `CoursePath` metadata, then replaces only the `CourseModule` rows included in the package and their child lessons. Modules that are not present in the package remain intact.

Small Course batches may therefore import one reviewed module without deleting unrelated local modules. Full-path packages should still include the full reviewed path tree when the intent is to refresh the entire level, because deletion of removed modules is intentionally not implicit without a future explicit full-replace mode.
