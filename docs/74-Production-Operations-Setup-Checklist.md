...
- [ ] Add the current server/operator IP in Brevo Authorized IPs at `https://app.brevo.com/security/authorised_ips`.
  - Current status: this gate reopened because Brevo now rejects API calls from the current host with `401 Unauthorized` / unrecognized IP. Use the latest `artifacts/validation/web-external-action-packet/` report to copy the IP value without pasting API keys or webhook tokens into Git-tracked files.
- [ ] Rerun `Invoke-BrevoProductionReadinessCheck.ps1` with `-VerifyBrevoApi` and confirm `brevo.accountApi` passes before real inbox/webhook smoke.
  - Current status: `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-222427.md` was run with `-SendingDomain darwinlingua.com -RequireRealDelivery -VerifyBrevoApi` and reports `Blockers=1`, `Warnings=0`; the only blocker is Brevo Authorized IP.
