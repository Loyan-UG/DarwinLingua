...
- [ ] Add the current server/operator IP in Brevo Authorized IPs at `https://app.brevo.com/security/authorised_ips`.
  - Current status: this gate reopened because Brevo now rejects API calls from the current host with `401 Unauthorized` / unrecognized IP. Brevo's current API/security documentation says IP security can block requests from unauthorized IPs even when the API key is valid, and manually authorized IPs apply to both API and SMTP keys. Use the latest `artifacts/validation/web-external-action-packet/` report to copy the IP value without pasting API keys or webhook tokens into Git-tracked files.
- [ ] Rerun `Invoke-BrevoProductionReadinessCheck.ps1` with `-VerifyBrevoApi` and confirm `brevo.accountApi` passes before real inbox/webhook smoke.
  - Current status: `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-232247.md` was run with `-RequireRealDelivery -VerifyBrevoApi -SenderVerified -DnsAuthenticated -WebhookConfigured -DpaAccepted` and reports `Blockers=1`, `Warnings=0`; the only blocker is Brevo Authorized IP.
  - Historical evidence: `artifacts/validation/brevo-readiness/brevo-production-readiness-20260623-185124.md` previously passed with `Blockers=0` and `Warnings=0`, but it no longer proves reachability from the current host/IP.
...
- [x] Verify email-change links use `TransactionalEmail__PublicBaseUrl`.
  ...
  - Public-stack evidence: `tools/Web/Invoke-WebAccountEmailLinkSmoke.ps1` resolved the real Brevo-tracked action links from delivered email content and confirmed the final links use `https://darwinlingua.com`.
...
- [x] Confirm the registration email link works.
  - Evidence: `artifacts/validation/web-account-email-link-smoke/web-account-email-link-smoke-20260623-162504.md` resolved the resent Brevo confirmation link, followed the final `https://darwinlingua.com` Identity confirmation URL, and verified `EmailConfirmed=true` for the test account.
...
- [x] Change account email and confirm both new-email confirmation and old-email notification are sent.
  - Evidence: the same public link smoke changed the test account email, resolved the new-email confirmation link, verified the account moved to the changed address, and verified `Account.EmailChangedNotification` was logged for the old address.
