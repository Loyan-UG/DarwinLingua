...
Current operational status for the controlled public Web stack: the Brevo API key and webhook secret are configured outside source control, the configured webhook secret matches the operator-provided 64-character token without being committed to Git, `no-reply@darwinlingua.com` exists as the sender, `support@darwinlingua.com` forwards to the main `info@darwinlingua.com` mailbox, the sender/domain readiness gate passes with zero blockers/warnings, DPA is accepted, direct Brevo real-delivery smoke passes, app-level registration/password-reset sends are logged through provider `brevo-api`, real Brevo-tracked registration/resend-confirmation/password-reset/email-change action links resolve and complete successfully on `https://darwinlingua.com`, the 2026-06-23 `web-account-email-link-smoke-20260623-173710.md` run passed after the branded HTML email layout upgrade, a controlled public webhook smoke verifies Bearer-authenticated `hardBounce` handling plus internal suppression creation, the Brevo transactional log check passes against provider logs, and a controlled suppressed-send smoke verifies that a later account email to a suppressed recipient is logged as `Suppressed` without calling Brevo. Manual mailbox rendering review still remains before broad public launch.
...
9. Set the Brevo webhook event category to `Transactional email` and select the official transactional events Brevo exposes in the current UI: request/sent, delivered, deferred, soft bounce, hard bounce, blocked, invalid email, spam, opened, unique opened, click/clicked, and unsubscribed. Brevo's create/update webhook API currently lists these transactional event values: `sent` or `request`, `delivered`, `hardBounce`, `softBounce`, `blocked`, `spam`, `invalid`, `deferred`, `click`, `opened`, `uniqueOpened`, and `unsubscribed`. The transactional webhook guide also documents provider events such as `Error`, `Proxy Open`, and `Unique Proxy Open`; the currently configured Brevo webhook was verified through the Brevo API with `bearer` auth and active events including `request`, `delivered`, `hardBounce`, `softBounce`, `blocked`, `spam`, `invalid`, `deferred`, `click`, `opened`, `uniqueOpened`, `proxyOpen`, `uniqueProxyOpen`, `error`, and `unsubscribed`. Darwin Lingua normalizes common camelCase/snake_case labels and equivalent provider failure reasons before storing them. Do not treat `complaint` as a required UI checkbox; if Brevo sends a complaint-like event or reason, the application still handles it as a permanent suppression signal.
...
- [x] password reset token resets password successfully
  - Evidence: `artifacts/validation/web-account-email-link-smoke/web-account-email-link-smoke-20260623-173710.md` resolved the Brevo-tracked password-reset link, posted a new password, and logged in with the reset password.
