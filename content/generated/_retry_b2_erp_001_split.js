...
const basePath=path.join(outDir,`de-${levelLower}-generated-${batch}.json`);
...
const wordPkg={...base,packageId:`de-${levelLower}-generated-${batch}-words`,packageName:`German ${level} ERP Generated Batch 001 Words`,collections:[]};
fs.writeFileSync(wordPath,JSON.stringify(wordPkg,null,2),'utf8');
const project=path.join(root,'src','Apps','DarwinLingua.ImportTool','DarwinLingua.ImportTool.csproj');
function run(file){ const res=cp.spawnSync('dotnet',['run','--project',project,'--','--target','shared','--yes',file],{encoding:'utf8',cwd:root}); const output=`${res.stdout||''}${res.stderr||''}`; process.stdout.write(output); return {status:res.status,output}; }
...
const collectionPkg={...base,packageId:`de-${levelLower}-generated-${batch}-collection`,packageName:`German ${level} ERP Generated Batch 001 Collection`,entries:[],labels:[],collections:base.collections};
fs.writeFileSync(collectionPath,JSON.stringify(collectionPkg,null,2),'utf8');
const collectionImport=run(collectionPath);
const collectionOk=collectionImport.status===0&&collectionImport.output.includes('Entries invalid: 0')&&collectionImport.output.includes('Warnings: 0')&&collectionImport.output.includes('Files failed: 0');
