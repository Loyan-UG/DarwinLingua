...
internal sealed class UnifiedLearningSearchRepository(IDbContextFactory<DarwinLinguaDbContext> dbContextFactory) : IUnifiedLearningSearchRepository
{
    ...
    public async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchAsync(UnifiedLearningSearchFilterModel filter, CancellationToken cancellationToken)
    {
        ...
        string targetLanguageCode = TargetLearningLanguageScope.NormalizeOrDefault(filter.TargetLearningLanguageCode);
        ...
        if (ShouldSearch(resultType, "word")) results.AddRange(await SearchWordsAsync(dbContext, query, targetLanguageCode, cefrLevel, category, topicKey, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "grammar")) results.AddRange(await SearchGrammarAsync(dbContext, query, targetLanguageCode, cefrLevel, category, topicKey, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "expression")) results.AddRange(await SearchExpressionsAsync(dbContext, query, targetLanguageCode, cefrLevel, category, topicKey, filter.IncludeSensitiveEducationalLanguage, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "dialogue")) results.AddRange(await SearchDialoguesAsync(dbContext, query, targetLanguageCode, cefrLevel, category, topicKey, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "roleplay")) results.AddRange(await SearchRoleplaysAsync(dbContext, query, targetLanguageCode, cefrLevel, category, topicKey, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "talk-topic")) results.AddRange(await SearchTalkTopicsAsync(dbContext, query, targetLanguageCode, cefrLevel, category, topicKey, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "exercise")) results.AddRange(await SearchExercisesAsync(dbContext, query, targetLanguageCode, cefrLevel, category, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "course-lesson")) results.AddRange(await SearchCourseLessonsAsync(dbContext, query, targetLanguageCode, cefrLevel, category, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "exam-prep")) results.AddRange(await SearchExamPrepAsync(dbContext, query, targetLanguageCode, cefrLevel, category, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "writing-template")) results.AddRange(await SearchWritingTemplatesAsync(dbContext, query, targetLanguageCode, cefrLevel, category, cancellationToken).ConfigureAwait(false));
        if (ShouldSearch(resultType, "country-guidance")) results.AddRange(await SearchCountryGuidanceNotesAsync(dbContext, query, targetLanguageCode, cefrLevel, category, cancellationToken).ConfigureAwait(false));
    ...
    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchWordsAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, string? topicKey, CancellationToken cancellationToken)
    {
        LanguageCode languageCode = LanguageCode.From(targetLanguageCode);
        IQueryable<WordEntry> words = dbContext.WordEntries.AsNoTracking().Where(word =>
        ...
        return await words
            ...
            .Select(word => new UnifiedLearningSearchResultModel("word", word.Lemma, word.PartOfSpeech.ToString(), word.PrimaryCefrLevel.ToString(), word.PartOfSpeech.ToString(), word.Topics.Join(dbContext.Topics, link => link.TopicId, topic => topic.Id, (link, topic) => topic.Key).ToArray(), BuildLearnUrl(targetLanguageCode, "words", LemmaUrlSlug.FromLemma(word.Lemma)), Score(query, word.Lemma, word.NormalizedLemma, null, 0), MatchFields(query, word.Lemma, word.NormalizedLemma, null)))
            .ToListAsync(cancellationToken)
            .ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchGrammarAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, string? topicKey, CancellationToken cancellationToken)
    {
        IQueryable<GrammarTopic> items = dbContext.GrammarTopics.AsNoTracking().Where(item =>
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("grammar", item.Title, item.ShortDescription, item.CefrLevel.ToString(), item.GrammarCategory, item.Topics.Join(dbContext.Topics, link => link.TopicId, topic => topic.Id, (link, topic) => topic.Key).ToArray(), BuildLearnUrl(targetLanguageCode, "grammar", item.Slug), Score(query, item.Title, item.Slug, item.ShortDescription, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.ShortDescription))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchExpressionsAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, string? topicKey, bool includeSensitiveEducationalLanguage, CancellationToken cancellationToken)
    {
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("expression", item.ExpressionText, item.ActualMeaningText, item.CefrLevel.ToString(), item.ExpressionType, item.Topics.Join(dbContext.Topics, link => link.TopicId, topic => topic.Id, (link, topic) => topic.Key).ToArray(), BuildLearnUrl(targetLanguageCode, "expressions", item.Slug), Score(query, item.ExpressionText, item.Slug, item.ActualMeaningText, item.SortOrder), MatchFields(query, item.ExpressionText, item.Slug, item.ActualMeaningText))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchDialoguesAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, string? topicKey, CancellationToken cancellationToken)
    {
        IQueryable<DialogueLesson> items = dbContext.DialogueLessons.AsNoTracking().Where(item =>
            item.PublicationStatus == PublicationStatus.Active &&
            item.TargetLearningLanguageCode == targetLanguageCode);
        if (cefrLevel.HasValue) items = items.Where(item => item.CefrLevel == cefrLevel.Value);
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("dialogue", item.Title, item.Description, item.CefrLevel.ToString(), item.Category, item.Topics.Join(dbContext.Topics, link => link.TopicId, topic => topic.Id, (link, topic) => topic.Key).ToArray(), BuildLearnUrl(targetLanguageCode, "dialogues", item.Slug), Score(query, item.Title, item.Slug, item.Description, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.Description))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchRoleplaysAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, string? topicKey, CancellationToken cancellationToken)
    {
        IQueryable<RoleplayScenario> items = dbContext.RoleplayScenarios.AsNoTracking().Where(item =>
            item.PublicationStatus == PublicationStatus.Active &&
            item.TargetLearningLanguageCode == targetLanguageCode);
        if (cefrLevel.HasValue) items = items.Where(item => item.CefrLevel == cefrLevel.Value);
        ...
        if (topicKey is not null) items = items.Where(item => item.Topics.Any(link => dbContext.Topics.Any(topic => topic.Id == link.TopicId && topic.Key == topicKey)));
        items = items.Where(item => EF.Functions.ILike(item.Title, $"%{query}%") || EF.Functions.ILike(item.Description, $"%{query}%") || EF.Functions.ILike(item.LearnerGoal, $"%{query}%") || EF.Functions.ILike(item.Slug, $"%{query}%"));
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("roleplay", item.Title, item.LearnerGoal, item.CefrLevel.ToString(), item.TaskType, item.Topics.Join(dbContext.Topics, link => link.TopicId, topic => topic.Id, (link, topic) => topic.Key).ToArray(), BuildLearnUrl(targetLanguageCode, "roleplays", item.Slug), Score(query, item.Title, item.Slug, item.LearnerGoal, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.LearnerGoal))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchTalkTopicsAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, string? topicKey, CancellationToken cancellationToken)
    {
        IQueryable<TalkTopic> items = dbContext.TalkTopics.AsNoTracking().Where(item =>
            item.PublicationStatus == PublicationStatus.Active &&
            item.TargetLearningLanguageCode == targetLanguageCode);
        if (cefrLevel.HasValue) items = items.Where(item => item.CefrLevel == cefrLevel.Value);
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("talk-topic", item.Title, item.Description, item.CefrLevel.ToString(), item.Category, item.Topics.Join(dbContext.Topics, link => link.TopicId, topic => topic.Id, (link, topic) => topic.Key).ToArray(), BuildLearnUrl(targetLanguageCode, "talk-topics", item.Slug), Score(query, item.Title, item.Slug, item.Description, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.Description))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchExercisesAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, CancellationToken cancellationToken)
    {
        IQueryable<Exercise> items = dbContext.Exercises.AsNoTracking().Where(item =>
            item.PublicationStatus == PublicationStatus.Active &&
            item.TargetLearningLanguageCode == targetLanguageCode);
        if (cefrLevel.HasValue) items = items.Where(item => item.CefrLevel == cefrLevel.Value);
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("exercise", item.Title, item.Instruction, item.CefrLevel.ToString(), item.ExerciseType, Array.Empty<string>(), BuildLearnUrl(targetLanguageCode, "exercises", item.Slug), Score(query, item.Title, item.Slug, item.Instruction, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.Instruction))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchCourseLessonsAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, CancellationToken cancellationToken)
    {
        IQueryable<CourseLesson> items = dbContext.CourseLessons.AsNoTracking().Where(item =>
            item.PublicationStatus == PublicationStatus.Active &&
            item.TargetLearningLanguageCode == targetLanguageCode);
        if (cefrLevel.HasValue) items = items.Where(item => item.CefrLevel == cefrLevel.Value);
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("course-lesson", item.Title, item.ShortDescription, item.CefrLevel.ToString(), item.CoursePathSlug, Array.Empty<string>(), $"/learn/{targetLanguageCode}/courses/{item.CoursePathSlug}/{item.Slug}", Score(query, item.Title, item.Slug, item.ShortDescription, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.ShortDescription))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchExamPrepAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, CancellationToken cancellationToken)
    {
        IQueryable<ExamPrepUnit> items = dbContext.ExamPrepUnits.AsNoTracking().Where(item =>
            item.PublicationStatus == PublicationStatus.Active &&
            item.TargetLearningLanguageCode == targetLanguageCode);
        if (cefrLevel.HasValue) items = items.Where(item => item.CefrLevel == cefrLevel.Value);
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("exam-prep", item.Title, item.ShortDescription, item.CefrLevel.ToString(), item.ExamSection, Array.Empty<string>(), BuildLearnUrl(targetLanguageCode, "exam-prep", item.Slug), Score(query, item.Title, item.Slug, item.ShortDescription, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.ShortDescription))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchWritingTemplatesAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, CancellationToken cancellationToken)
    {
        IQueryable<WritingTemplate> items = dbContext.WritingTemplates.AsNoTracking().Where(item =>
            item.PublicationStatus == PublicationStatus.Active &&
            item.TargetLearningLanguageCode == targetLanguageCode);
        if (cefrLevel.HasValue) items = items.Where(item => item.CefrLevel == cefrLevel.Value);
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("writing-template", item.Title, item.ShortDescription, item.CefrLevel.ToString(), item.Category, Array.Empty<string>(), BuildLearnUrl(targetLanguageCode, "writing-templates", item.Slug), Score(query, item.Title, item.Slug, item.ShortDescription, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.ShortDescription))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }

    private static async Task<IReadOnlyList<UnifiedLearningSearchResultModel>> SearchCountryGuidanceNotesAsync(DarwinLinguaDbContext dbContext, string query, string targetLanguageCode, CefrLevel? cefrLevel, string? category, CancellationToken cancellationToken)
    {
        IQueryable<CountryGuidanceNote> items = dbContext.CountryGuidanceNotes.AsNoTracking().Where(item =>
            item.PublicationStatus == PublicationStatus.Active &&
            item.TargetLearningLanguageCode == targetLanguageCode);
        if (cefrLevel.HasValue) items = items.Where(item => item.CefrLevel == cefrLevel.Value);
        if (category is not null) items = items.Where(item => item.Category == category);
        ...
        return await items.OrderBy(item => item.SortOrder).Take(MaxResultsPerType).Select(item => new UnifiedLearningSearchResultModel("country-guidance", item.Title, item.ShortDescription, item.CefrLevel.ToString(), item.Category, Array.Empty<string>(), BuildLearnUrl(targetLanguageCode, "country-guidance/" + item.CountryContextCode.ToLower(), item.Slug), Score(query, item.Title, item.Slug, item.ShortDescription, item.SortOrder), MatchFields(query, item.Title, item.Slug, item.ShortDescription))).ToListAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    private static string BuildLearnUrl(string targetLanguageCode, string segment, string slug) =>
        $"/learn/{targetLanguageCode}/{segment}/{slug}";
